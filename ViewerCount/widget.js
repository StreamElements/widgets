/*
Change values below to fit your needs (remember that refresh ratio should not be too low). 

If you want other styling:
1) go to https://github.hubspot.com/odometer/api/themes/
2) pick on theme name (for example blue "train-station")
3) copy content of opened style to CSS tab in Custom CSS

*/


let timeToRefresh = 60;
let channelName,provider,providerId;
...
function getData() {
    if (provider === "twitch") {
        fetch(`https://decapi.me/twitch/viewercount/${channelName}`).then(response => response.text()).then(count => {
          if (isNaN(parseInt(count))) count = 0; //check for 'user is offline'  
            $(".odometer").html(count);
        });
